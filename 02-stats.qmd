...
You can see from @tbl-tablevalues that if you want to make sure that exactly 95% of your data will fall in the given range, *i.e.* which defines the **95% confidence interval**, you need to use a range of $1.96\sigma$.

:::

::: {.callout-tip collapse="true"}

## Tip: compute the 95% CI in R

In R, you can compute the 95% CI of a distribution using `Rmisc::CI()`{.R} or `t.test()`{.R}.
...
#| code-fold: false
#| echo: false
TA <- t.test(temperatures %>% filter(Patient == "A", Day=="Day1") %>% pull(Temperature), 
       temperatures %>% filter(Patient == "A", Day=="Day2") %>% pull(Temperature))
TB <- t.test(temperatures %>% filter(Patient == "B", Day=="Day1") %>% pull(Temperature), 
...
temperatures %>% 
    ...
    mutate(CI = map(data, ~as_tibble(as.list(Rmisc::CI(.$Temperature,ci=.95))))) %>% 
    unnest(CI) %>% 
    select(Day, Patient, lower, mean, upper) %>% 
