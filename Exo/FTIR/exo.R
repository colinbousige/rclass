...
flist <- list.files(pattern= ????, path=????, full.names=????)
flist
...
blank.df <- read_excel(????, range = ????, col_names = ????)
raw.df <- ?????
alldata <- ????
alldata$transmittance <- ????
...
# In case you didn't manage to do the above procedure, just load the `alldata.csv` file in the `Data` folder by running the following command:
...
# - You see in the file names that there are actually 5 samples with 2 temperatures for each. Based on the column `name` of the `df` tibble, create the columns `sample` and `temperature` that contain *numerical values* of the sample number and temperature. Think about using the function `gsub()` to replace (delete) text in a string, the `substr(string, start, end)` function to get a portion of a string. You can also take a look at the `separate()` function from the `tidyverse`.
