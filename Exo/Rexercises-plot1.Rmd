...
## First things first
The first thing to do is setting up your working environment path, and load various libraries:

```{r, message=FALSE, warning=FALSE}
# set working directory 
# (replace path by your own, where you put the "Data" folder)
...
## Loading data
To load a column file into a `data.frame`:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
# The file is text
df1 <- read.table("Data/test.dat")
df1
# read first line as column names
df1 <- read.table("Data/test.dat", header=TRUE)
df1
# The file is an Excel file
df2 <- read_excel("Data/test.xlsx")
df2
```

To access/change the names of the columns:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
names(df1)
# change the names
names(df1) <- c("w","Int")
...
To get a column or a row:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
# equivalent call to columns by name or index:
df1$w
df1[["w"]]
df1[[1]]
# call to rows by index
df2[1:3,]
df2[1:3,"y"] #careful, subsetting a tibble returns a tibble
df2$y[1:3]
```

## Plotting
To plot, simply use `plot()`, and type `?plot` to see the options. Add lines with `lines()` and points with `points()`, add straight lines with `abline()`:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
plot(df2$x, df2$y, 
    type="l",
    xlab="X label",
...
lines(df1$w, df1$Int, col="red")
points(df2$y, df2$x, col="blue", pch=16)
abline(h=6)
abline(v=3)
```

Do an equivalent (but nicer) plot with `ggplot2`:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
library(ggplot2)
ggplot(data=df2)+
    geom_line(aes(x=x,y=y))+
    geom_point(aes(x=y,y=x), col="blue")+
    geom_line(data=df1, aes(x=w,y=Int), col="red")+
...
A function returns the last thing that is called
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
# This function returns a vector normalized to 1
norm1 <- function(x){
    x/max(x)
}
x <- seq(1,5,1)
...
## For loops
`For` loops can increment along a numerical vector, or any vector:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE}
brands <- c("Ford","Renault","Citroen","Toyota")
...
This can be useful for a recursive plot, for example. First, you initialize an empty plot, and use a `for` loop to incrementally add elements:
```{r include=TRUE, warning = FALSE, message=FALSE, cache=FALSE, fig.height=4.3}
xs <- seq(0,10,.1)
colors <- colorRampPalette(c("black","royalblue","orange","red"))(length(xs))
plot(NA,NA, #initialize an empty plot
    xlab="x",ylab="sin(x)",
    xlim=c(0,10),
    ylim=c(-1,1),
    las=1)
for (i in 1:length(xs)) {
    points(xs[i],sin(xs[i]),col=colors[i], pch=16, cex=2)
}
```
