...
- Now then, using the same procedure as in the example above, fit the total Confirmed cases for France, China, Italy and Spain using a [logistic function](https://en.wikipedia.org/wiki/Logistic_function), and plot the data and their fit, and show the results of the fits. Here is the expected plot: 
...
fits <- alldata %>% 
    filter(country %in% countries) %>% 
    group_by(country) %>% 
    do(fit=nls(confirmed ~ a/(1+exp(-b*(date-c))), 
               data = .,
