...
$endif$
$if(twocolumn)$
\author{$for(author)$\MakeUppercase{$author.name$}$if(author.affiliation)$\\\small{$author.affiliation$}$endif$$if(author.email)$\\\small{$author.email$}$endif$$sep$ \and $endfor$}
$else$
\author{$for(author)$\MakeUppercase{$author.name$}$if(author.email)$\footnote{~Email: $author.email$}$endif$$if(author.affiliation)$\\\small{$author.affiliation$}$endif$$sep$ \and $endfor$}
$endif$
\date{$date$}
$if(beamer)$
$if(institute)$
...
$if(title)$
\title{\Huge{\textcolor{BLUE}{\textbf{$title$}}}$if(subtitle)$\\ \LARGE{$subtitle$}$endif$}
$endif$ 
...
\titleformat*{\section}{\color{BLUE}\large\bfseries$if(sansitup)$\sffamily\uppercase$else$$endif$}
\titleformat*{\subsection}{\color{BLUE}$if(sansitup)$\bfseries\sffamily$else$\normalsize\itshape$endif$} % \small\uppercase
\titleformat*{\subsubsection}{\color{BLUE}\normalsize\itshape}
\titleformat*{\paragraph}{\color{BLUE}\normalsize\itshape}
\titleformat*{\subparagraph}{\color{BLUE}\normalsize\itshape}
...
\renewenvironment{abstract}{
  \begin{@bstr@ctlist}\begin{tcolorbox}[halign=justify,arc=0pt,colback=BLUE!5!white,colframe=white!0!white]\abstracttextfont\normalsize}%
...
$else$
    ...
    \begin{abstract}
        \begin{tcolorbox}[halign=justify,arc=0pt,colback=BLUE!5!white,colframe=white!0!white]
        % \rule{\linewidth}{.5pt}
    %    \hbox{\vrule height .2pt width 39.14pc}
