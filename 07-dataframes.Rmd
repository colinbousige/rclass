...
- Then, to read this file into a `data.frame`, we will use `read.table()`{.R}. If you don't specify that the file contains a header, `read.table()`{.R} will default to attirbuting column names that will be V1, V2, V3, etc:
```{r}
...
- Now, to read an Excel file, use the `read_excel` library:
```{r}
...
```
- Or by its name using the `$` notation:
```{r, warnings=FALSE}
df$X
```
...
> **Attention:** the two data.frame must have the same number of columns _**and**_ the same column names.
...
Take a look at the [cheatsheet on the `purrr` package](https://github.com/rstudio/cheatsheets/raw/master/purrr.pdf) for more options. I show here a use of `purrr::map(vector, function)`{.R} that returns a list. `map(x, f)`{.R} applies the function `f()` to each element of the vector `x`, putting the result in a separate element of a list: `map(x, f) -> list(f(x1), f(x2), ... f(xn))`{.R}. In case `f(xi)` returns a single value, you might want to use `map_dbl()` or `map_chr()`, for example, that will return a vector of doubles or of characters, respectively.
...
- Create a `data.frame`{.R} (or a `tibble`{.R}) called `people` from <a href="Data/people.txt" download target="_blank">people.txt</a>. Try using the function `read.table` or its `tidyverse` equivalent `read_table`.
