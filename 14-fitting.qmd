...
It might sound like a trivial question, but after years of teaching this class, I have realized it is not such an evident concept to all students. So, let's take a look at some experimental measurements of the oscillation period *T* of a pendulum. We have measured this period for various lengths *L* of the pendulum, and it results in such an evolution:

```{r}
#| label: fig-somedata
#| fig-cap: "Some experimental data showing the period of a pendulum measured as a function of its lenght."
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
period <- function(l,g){
...
- **Parameters:** the gravity *g* is a parameter of the fit that we wich to determine, *i.e.* a constant.

So now, what exactly *is* a fit?

Fitting a model to experimental data means **finding the parameters of the model that best describe our data**. How do we know the parameters are the best ones to describe our data? For this, we use a tool called the residual error function, $\chi^2$, such as:
...
P1 <- df %>% 
    ggplot(aes(x = l, y = T)) +
       ...
       labs(title = substitute(paste("g = ",~g,~m^{2},"/s"), list(g=g)),
            subtitle = substitute(paste(chi^2,"= ",~chi1), list(chi1=chi1)),
...
P2 <- df %>% 
    ggplot(aes(x = l, y = T)) +
       ...
       labs(title = substitute(paste("g = ",~g,~m^{2},"/s"), list(g=7)),
            subtitle = substitute(paste(chi^2,"= ",~chi2), list(chi2=chi2)),
            x= "Pendulum length L [m]",
            y= "Period [s]")
P3 <- df %>% 
    ggplot(aes(x = l, y = T)) +
       ...
       labs(title = substitute(paste("g = ",~g,~m^{2},"/s"), list(g=11)),
            subtitle = substitute(paste(chi^2,"= ",~chi3), list(chi3=chi3)),
            x= "Pendulum length L [m]",
            y= "Period [s]")
P4 <- df %>% 
    ggplot(aes(x = l, y = T)) +
       ...
       labs(title = substitute(paste("g = ",~g,~m^{2},"/s"), list(g=6)),
            subtitle = substitute(paste(chi^2,"= ",~chi4), list(chi4=chi4)),
            x= "Pendulum length L [m]",
            y= "Period [s]")
P4+P2+P1+P3
```
...
## Important
Fitting a model to experimental data is finding this sweet spot for the ensemble of parameters.
:::
...
- When your experimental data follow a linear trend: `lm()`{.R}
- The rest of the time: `nls()`{.R}
