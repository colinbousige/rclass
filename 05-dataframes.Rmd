...
A `tibble`{.R} is an enhanced version of the `data.frame`{.R} provided by the `tidyverse` package. The main advantage of `tibble`{.R} is that it has easier initialization and nicer printing than `data.frame`{.R}, and the performance are also enhanced – especially for the reading from files with `read_delim()`{.R}, `read_tsv()`{.R}, `read_table()`{.R} and `read_csv()`{.R}. Otherwise, the handling is basically the same.
...
dt6 <- tibble(file=list.files(path="Exo/spectro/Data/", pattern = ".txt")) %>% 
    mutate(data = map(file, 
                      ~read_table(file.path("Exo/spectro/Data", .),
                                   col_names = c("w", "int"))
                      ),
