...
Death_raw<- read_csv("time_series_covid19_deaths_global.csv")
```
...
- Using successive operations with the pipe operator `%>%`{.R} create a function `tidyfy_jhh(df, name)`{.R}, that, given a tibble `df` (_i.e._, the data from JHH we just loaded), will:
    ...
    + Using the ` `{.R} function from the `lubridate` package, convert the `date` column to a R Date format
...
tidyfy_jhh <- function(df, name) {
  df %>%
    ...
    summarize(!!sym(name):= sum(your_result_here))
}
...
Deaths <- tidyfy_jhh(Death_raw, Deaths)
```
...
covid <- covid %>% group_by(country) %>%
  summarise(mratio=mean(ratio))

w <- covid %>%
  ggplot(aes(x=reorder(country, -(mratio)), 
                   y=mratio, 
                   fill=mratio)) +
  geom_bar(stat="identity") +
  ggtitle("ratio per country")+
...
m <- covid2 %>%
  ...
  geom_bar(stat="identity") +
  ggtitle("newcases per day")+
            labs(x="day", y="newcases")

m + geom_vline(xintercept=11, color = "red") +
