...
- Unzip the `Exam_2020.zip` file somewhere on the computer, and rename the Rmd file as *name_firstname.Rmd*. You will work in this Rmd file and send it to me by email at the end, at **colin.bousige@univ-lyon1.fr**. Change the name of the author in the YAML header to your own name, but don't touch anything else. You will get a bonus if the file you sent me can be knitted without error.
- Load the libraries `tidyverse` and `broom` and set the global `ggplot2` theme to `theme_bw()`.
...
# Exercise 1 - Starwars (5 points)

- Look at the help on the built-in `starwars` tibble.

```{r include=params$solution, warning=FALSE, message=FALSE}
?starwars
```

- Add a column `BMI` containing the body mass index of each character. It is defined as $BMI=mass[kg]/height[m]^2$.
...
- Display the average BMI per species, and remove the lines containing `NA` values. What are the species with the maximum and minimum average BMI?

```{r include=params$solution, warning=FALSE, message=FALSE}
(starwars_ave <- starwars %>% 
    group_by(species) %>% 
    summarize(BMI_average = mean(BMI)) %>% 
    na.omit())
starwars_ave %>% filter(BMI_average %in% range(BMI_average))
```

- Using the whole `starwars` tibble, plot the mass as a function of the height, with a different color per gender.
...
- Remove the outlier, and make again the same plot
...
- Make a linear fit of the mass as a function of the height (all genders aside) and display the slope and intercept, their errors, and the plot with the linear fit.
...
- Define the `Murnaghan(P, K, Kp)` function that, given the pressure $P$, the bulk modulus $K$ and its derivative $Kp$ (defaulting to $Kp=4$), returns the relative volume $\frac{V}{V_0}$
...
- Find in the `Data` folder all the files containing the experimental diffraction patterns. They are under the form `PY02_Pxx.dat` or `PY02bis_Pxx.dat`. Store this list of files in a vector called `flist`.

```{r include=params$solution, warning=FALSE, message=FALSE}
flist  <- list.files(path="Data", pattern = "_P")
```

- Read and store the two files containing the pressures of each run, `PY02_pressures.dat` and `PY02bis_pressures.dat`. Combine these two tables into a single tidy one called `Pressures`, containing three columns `run`, `P.kbar` and `sample`.
...
- Define the `get_run(flist)` function that, given **a vector** of file names, returns **a vector** of the run number of the corresponding files. The output pressure must be given in **GPa**. The files are under the form `PY02_Pxx.dat` or `PY02bis_Pxx.dat`, where `xx` is the  run number.
...
- Plot all the diffractograms on top of each other, with lines of a different color for each file, and define nice axis labels, such as *Scattering Vector q [1/Å]* and *Intensity [arb. units]*.
...
- Store the resulting values of $K$ and it's fitting standard error $dK$ in two variables. Round the values to the appropriate floating number. You may use the `broom::tidy()` function.
...
pospeak %>%
    ggplot(aes(x=P, y=VV0))+
        ...
        theme(legend.position = c(.9,.85))->p
```
