...
```{r}
flist <- list.files(path = file.path("C:/Users/leodu/Desktop/Rfile/Dataexam"), pattern = ".dat")
flist <- flist[!(flist %in% c("PY02_pressures.dat","PY02bis_pressures.dat"))]
...
```{r}
PYO2 <- read_table2("C:/Users/leodu/Desktop/Rfile/Dataexam/PY02_pressures.dat") %>% mutate(sample = "PY02")
PYO2bis <- read_table2("C:/Users/leodu/Desktop/Rfile/Dataexam/PY02bis_pressures.dat") %>% mutate(sample = "PY02bis")
SUM <- rbind(PYO2,PYO2bis)
...
```{r}
readfile <- function(fille){
   read_table2(file.path("C:/Users/leodu/Desktop/Rfile/Dataexam",fille), col_names = c("q","int")) %>%
      mutate(int_n = norm01(int)) %>%
      mutate(file = fille)
}

data <- tibble(fichier=flist) %>% 
    mutate(data = map(fichier, ~readfile(.))) %>%
    unnest(data) %>%
    select(-fichier)
...
```{r}
P1 <- data %>% ggplot(aes(x = q, y = int_n, color = file)) +
   geom_line() +
   labs(x = "Scattering Vector q [1/Å]", y = "Intensity [arb. units]") +
   ggtitle("normalized diffractograms")
...
```{r}
pospeak <- pospeak %>% group_by(sample) %>%
   mutate(VVO = (Q[which(P.kbar == 0)]/Q)^3) %>%
   mutate(dVVO = 3*VVO*(Q/dQ))

##because It found that d(V/Vo) = 3(V/Vo)*(Q/dQ)
```
...
##we should work with P

fits <- pospeak %>% 
    nest(data=-sample) %>% 
    mutate(fit=map(data, ~ nlsLM(data = .,
               VVO ~ A*Murnagham(P,K,Kp), 
               start=list(A = 1,K = 3, Kp = 4))),
           tidied = map(fit, tidy),
...
```{r}
Kfit <- fits %>% unnest(tidied) %>% group_by(term)
   select(estimate,std.error,term) %>%
   pivot_wider(names_from = term, values_from = sample )

## I did half of the job here - > this will lead to a mess in the plot because I didnt well manage the data
```
...
fits %>% full_join(Kfit) %>% unnest(augmented) %>%
   group_by(sample) %>%
    ggplot(aes(x=P, y=.fitted, color=sample))+
        geom_point(alpha=.5) +
        geom_errorbar(aes(ymin= (1+P*4/(K-dK))^(-1/4) ,ymax=(1+P*4/(K+dK))^(-1/4)), width=.2) +
   geom_line(data = pospeak, aes(x = P, y = VVO, color = sample)) +
   labs(x = "Pressure [GPa]", y = "V/Vo")

##the problem here is that I have two values of K and dK for each sample, which lead to a mess in the graph...
```
