...
```{r}
flist <- list.files(path = file.path("Data"), pattern = ".dat")
...
```{r}
PYO2 <- read_table2("Data/PY02_pressures.dat") %>% mutate(sample = "PY02")
PYO2bis <- read_table2("Data/PY02bis_pressures.dat") %>% mutate(sample = "PY02bis")
...
readfile <- function(fille){
   read_table2(file.path("Data",fille), col_names = c("q","int")) %>%
