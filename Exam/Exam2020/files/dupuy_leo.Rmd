...
```{r}
PYO2 <- read_table("Data/PY02_pressures.dat") %>% mutate(sample = "PY02")
PYO2bis <- read_table("Data/PY02bis_pressures.dat") %>% mutate(sample = "PY02bis")
...
readfile <- function(fille){
   read_table(file.path("Data",fille), col_names = c("q","int")) %>%
