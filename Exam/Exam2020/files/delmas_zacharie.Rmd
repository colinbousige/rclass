...
```{r}
flist <- list.files(pattern = "PY", path = "data")
```
...
```{r}
Py0 <- read.delim("Data/PY02_pressures.dat")
Py1 <- read.delim("Data/PY02bis_pressures.dat")

Pressures <- Py0 %>%
   full_join(Py1) %>%
    separate(run....P.kbar, c("run","P.kbar","sample")) 
...
fit <- nls(data=pospeak,
           VVO ~ Murnaghan(P, K, Kp),
           start = list(P=0, K=0, Kp=0))
```
...
```{r}
colors <- colorRampPalette(c("orange", "grey"))
pospeak %>% ggplot(aes(x=P, y=VV0, color=sample)) +
    geom_point() +
    labs(x = "Pressure [GPa]",
         y = "V/V0",
         color = "Sample")+
   
   ggtitle("Murnaghan fit : K = (X) +- (XX) GPa") +
   geom_errorbar(aes(ymin=VV0-dVV0,ymax=VV0+dVV0),width=.5) +
   theme(legend.position = c(0.9, 0.8))
#+   geom_line(aes(y=fit),linetype="dotted", color="blue")
```
