...
```{r}
Pressures1 <- read_table2("Data/PY02_pressures.dat") %>% mutate(sample= 'PY02')
Pressures2 <- read_table2('Data/PY02bis_pressures.dat') %>% mutate(sample= 'PY02bis')
Pressures <- Pressures2 %>%
   full_join(Pressures1)
...
data <- read_table2(flist)

data1 <- read_table2('Data/PY02_P1.dat')
...
x <- ggplot(data = data,
  ...
  ggtitle("diffractogram")+
  xlab("Scattering Vector q [1/Å]")+
