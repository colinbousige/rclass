...
library(data.table)
f1<- tibble(read_table2('Data/PY02_pressures.dat'))
f2<- tibble(read_table2('Data/PY02bis_pressures.dat'))

pressures<- data.table(f1,f2)
sample<- unlist(strsplit('PY02_pressures.dat','/'))
sample
sample<- gsub('.dat','',sample[1])
pressures$sample<- sample
pressures
```
...
readfile("Data/PY02_pressures.dat")

data<- tibble(file=flist)%>%
   mutate(mydata=map(file,~readfile(file.path('Data',.))))%>%

   unnest(mydata)
#data %>% rename(q= names(mydata)[2],int=names(mydata)[3])
#data%>% mutate(int_n=norm01(.$mydata[2]) )
...
```{r,echo=TRUE}
#separate(column_name,c('sample','run','extension'))
