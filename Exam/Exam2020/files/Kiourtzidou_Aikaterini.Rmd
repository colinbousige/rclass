...
```{r}
setwd("C:/Users/keit_/Desktop/exam2020")
...
flist <- list.files(path="Data", pattern="PY02_P")
```
...
t1 <- read_table2("Data/PY02_pressures.dat")
t1 <- t1 %>% mutate(sample = "PY02")
t1
t2 <- read_table2("Data/PY02bis_pressures.dat")
t2 <- t2 %>%
         mutate( sample ="PUY02bis")
t2

Pressures <- rbind(t1,t2)
Pressures
...
data <- tibble(file=flist) %>% 
        mutate(q = map(file, ~read_table2(file.path("Data",.)))) %>%
        unnest(q) 
        

names(data) <- c("file","q","int")
data

data <- data %>% 
         mutate(int_n = norm01(data$int))
...
```{r}
data %>% 
    ggplot(aes(x=int_n, fill=factor(q)))+
