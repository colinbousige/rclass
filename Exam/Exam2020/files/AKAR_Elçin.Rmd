...
```{r}
#read_table is giving error I don't know why so I put comment on this code. It is supposed to work.
#data <- tibble(name=flist) %>% 
    #mutate(data=purrr::map(name, ~read_table(file.path("Data",.)), col_names= c("file", "q", "int"))) %>%
    #unnest(data) %>% 
    #select(-c(run,P.kbar))#%>%
    #rename("name"= "file",
         #"0.0019196879" = "q",
...
#read_table is giving error everywhere so I put comment.
