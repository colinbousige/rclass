...
library(minpack.lm)
theme_set(theme_bw())
# setwd("/home/marc/Documents/Elcin_folder/RCourse/Data/exam2020")
...
PY02bisPressure <- read.table("Data/PY02bis_pressures.dat")
PY02bisPressure <- tibble(PY02bisPressure)

PY02Pressure <- read.table("Data/PY02_pressures.dat")
PY02Pressure <- tibble(PY02Pressure)

Pressures <- bind_rows(PY02bisPressure, PY02Pressure) %>%
...
```{r}
#Read_table2 is giving error I don't know why so I put comment on this code. It is supposed to work.
#data <- tibble(name=flist) %>% 
    #mutate(data=purrr::map(name, ~read_table2(file.path("Data",.)), col_names= c("file", "q", "int"))) %>%
    #unnest(data) %>% 
    #select(-c(run,P.kbar))#%>%
    #rename("name"= "file",
         #"0.0019196879" = "q",
         #"0" = "int") %>%
    #mutate(int_n= norm(int))
    #mutate(q= round(q, digits=3),
                    #int= col_double(int)) 
...
#read_table2 is giving error everywhere so I put comment.
...
```{r}
pospeak %>% mutate(VVO= Murnaghan(0, 1.802587, 4 ),
                   DVVO = Q-VVO)
```
...
#I could not get the Section 9, but here is how I would write the fit.
#fits <- pospeak %>% 
    #nest(data = -sample) %>%
    #mutate(fit = purrr::map(data, ~ nlsLM(data = .,
                                #VVO ~ A*Murnaghan(P, K, Kp=4), 
                                #start=list(A = 1,
                                           #K= 1,
                                           #Kp=4,
                                #))),
...
#fits %>% unnest(augmented) %>%
  #ggplot(aes(x=P, y=VVO, color=sample)) +
  #labs(x= "Pressure[GPa]",
       #y= "V/V0",
       #color= "Sample")+
