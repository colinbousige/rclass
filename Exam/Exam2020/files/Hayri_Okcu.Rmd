...
```{r}
PY02_P <- list.files("Data", pattern="PY02_P");PY02_P
PY02bis_P <- list.files("Data", pattern="PY02bis_P"); PY02bis_P
flist <- c(PY02bis_P,PY02_P); flist
```
...
```{r}
PY02_pressures <- read_table2("Data/PY02_pressures.dat")
PY02_bispressures <- read_table2("Data/PY02bis_pressures.dat")

PY02_pressures$sample <- "PY02"
PY02_bispressures$sample <- "PY02bis"

Pressures <- rbind(PY02_pressures,PY02_bispressures)
```
...
data <- tibble()
for (file in flist) {
  d <- read_table2(paste0("Data/",file), col_names=TRUE)
  d$file <- file
  d$int_n <- norm01(d$"0")
  data <- bind_rows(data, d)
}
names(data) <- c("q","int","file","int_n")
```
...
```{r}
fnames <- unique(data$file)

data %>%
  ggplot(aes(
    x=q,
    y=int_n-1+as.numeric(factor(file)),
    col=file
  )) +
  geom_line() +
...
pospeak <- data %>%
  ...
  filter(q < 2.5) %>%
  filter(int == max(int)) %>%
  mutate(Q = q[which.max(int)]) %>%
  mutate(dQ = 0.005*q) %>%
  separate(file, c("sample","run","extension")) %>%
  mutate(run = gsub("P","",run)) %>%
  select(-c("extension","int","q","int_n")) %>%
  ungroup()
pospeak
```
...
```{r}
pospeak$run <- as.numeric(as.character(pospeak$run))

pospeak <- inner_join(pospeak,Pressures, by = c("run","sample"))
pospeak <- mutate(pospeak,P = pospeak$P.kbar/10)
pospeak
```
...
pospeak <- pospeak %>%
  ...
  mutate(r0 = 3.5,
         V0 = 3.5^3) %>%
  mutate(VV0 = V0*((r/r0)^3)) %>%
  mutate(dVV0 = V0*(((dQ/(2*pi))/r0)^3 )) %>%
  select(-c("r","r0","V0"))
```
