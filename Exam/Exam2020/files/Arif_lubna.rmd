...
Murnaghan<- function(P,K,Kp=4){
   rv=(1+(P*Kp)/K)^(1/Kp)   #rv is  the relative volume
   rv
}
```
...
```{r}
#Storing the files in flist variable
pattern1 = c("PY02_","PY02bis_.dat")
flist<-list.files(pattern="PY02",
                  path="C:\\Users\\Lubna\\Downloads\\Datas")  
flist
...
```{r}
#rock stores the files with the name "PY02_pressures.dat"and "PY02bis_pressures.dat"
rock<-tibble()
for (file in flist){
  d <- read_table2(file.path("Datas",file), col_names=FALSE)
  names(d)<-c("run","P.kbar")
  d<-d%>%
     mutate(samples=file)
  rock<-rbind(rock,d)
  
}
#to store the files with the names "PY02_pressures.dat"and "PY02bis_pressures.dat"
rock1<-rock%>%
   filter(samples==c("PY02_pressures.dat","PY02bis_pressures.dat"))
...
```{r}
#x is the input value which we want to normalize
norm01<- function(x){
x-min(x)/max(x)-min(x)
}
...
names(data)<-c("run","P.kbar","samples")

##problem is the data becomes character after coming out of the loop
```
...
```{r}
#to add a new column with normalized values
data$norm<-norm01(data$P.kbar)
data%>%
   ggplot(aes(x=run,y=norm,col=samples))+geom_point(alpha=0.2,size=5)+
   labs(x="Scattering Vector q [1/Å]",y="Intensity [arb. units]")+
   ggtitle("normalized diffrctograms")
```
...
newdata<-data%>%
   group_by(samples)%>%
   maxim<-which.max(data$P.kbar)%>%  #finding the maximum intensity
   maxi<-data$run[maxim]%>%
   summarise(maximum_int=max)
...
inner_join(newdata,data,by="P.kbar")
```
...
PY02bis   4 1.933126 0.009665629  38.00 3.800
PY02bis   5 1.971520 0.009857598  48.00 4.800")
#rv is the relative volume
pospeak$rv<-Murnaghan(pospeak$P,pospeak$P.kbar)

d_fit <- pospeak %>% 
    nest(data=-sample) %>% 
    mutate(fit=map(data, ~ nlsLM(data = .,
                   rv~P*A+B, start=list(a=max(.$P), b=.2, c=10))),

                   start=list(A=1, B=1))),
           tidied = map(fit, tidy),
           augmented = map(fit, augment))
d_fit %>% unnest(augmented)
ggplot(aes(x=P, color=sample))+
        geom_point(aes(y=rv), alpha=.2, size=3)+
        geom_line(aes(y=.fitted))+labs(x="Pressure[GPa]",y="V/Vo"))
#unnesting
d_fit %>% unnest(tidied)
d_fit %>% unnest(tidied) %>% 
...
ggplot(data=d, aes(x=x, y=y, col=sample))+
    geom_point(alpha=0.5) + 
    geom_line(data=d_fit %>% unnest(augmented), 
        aes(x=x,y=.fitted, col=sample))
