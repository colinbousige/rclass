...
library(ggplot2)
df <- trees
df <- data.frame(Height=x, Volume=y)
plot(df, type="l")

df <- data.frame(Girth=x, Volume=y)
plot(df, type="l")
...
```{r}
setwd("/home/adebyteadam/Documents/Rlanguage/Exam/Exam")
df1 <- USArrests
df1
...
```{r}
summary(df1)
#Average murder rate in the whole country is 7.788 i.e the mean value
  ```

> What is the state with the highest assault rate?

```{r}
  # State with highest Assault rate =  NORTH CAROLINA with rate 337
...
colnames(df1) <- c("Murder","Assault","Urbanpop","Rape")
View(df1)
```
