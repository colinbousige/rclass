...
```{r}
plot_height <- ggplot(data=trees, aes(x=Height,y=Girth,))+
  geom_point()+
...
plot_volume <- ggplot(data=trees, aes(x=Volume,y=Girth,))+
  geom_point()+
  geom_smooth(method = 'lm')
...
fit_height<- lm(y~x, data = trees)

summary(fit_height)

plot_height <- ggplot(data=trees, aes(x=Height,y=Girth,))+
  geom_point()+
  geom_smooth(col='red', lty=2)
...
max_assault <- max(USArrests$Assault)
...
new_us <-subset(USArrests,USArrests$UrbanPop >= 80 )
new_us
...
which(row.names(new_us$Rape)== 8.3)
...
new_us.1 <- new_us[order(new_us$UrbanPop,decreasing = TRUE), ]
new_us.1
...
new_us.2 <-new_us.1[order(new_us$Murder,decreasing = FALSE), ]
...
```{r}
Pressure <- function(w,l){
    (1876/10.71)(((l/694.24)^10.71)-1)
}
```
