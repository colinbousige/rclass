...
<details>
    <summary>**Exercise 3:** extension to a large number of files</summary>
...
- Plot the evolution of the position of the peaks as a function of the file name. Turn the file name by an angle of 45˚ for better visualization.
...
data %>% 
    ...
    ggplot(aes(x = w, y = Int)) +
       ...
       facet_wrap(~basename(file))
data %>% 
    unnest(augmented) %>% 
    ggplot(aes(x = w, y = Int + as.numeric(factor(file)), group=file)) +
       geom_point(alpha=0.5)+
       geom_line(aes(y = .fitted + as.numeric(factor(file))), col = "red")
P <- data %>%
    unnest(augmented) %>%
    ggplot(aes(x = w, y = Int, frame=basename(file))) +
        geom_point(alpha = 0.5) +
        geom_line(aes(y = .fitted), col = "red")
plotly::ggplotly(P, dynamicTicks = TRUE) %>% 
    animation_opts(5)%>%
    layout(xaxis = list(autorange=FALSE, range = c(3050, 3550)))
# - Plot the evolution of the position of the peaks.
# Turn the file name by an angle of 45˚ for better visualization.
data %>%
    unnest(tidied) %>% 
    filter(term == "x1") %>% 
    ggplot(aes(x = basename(file), y = estimate)) +
        geom_point()+
        geom_errorbar(aes(ymin = estimate - std.error,
                          ymax = estimate + std.error),
                     width=0.5)+
