...
In `nls()`{.R} it is even possible to constraint the fitting by adding lower and upper boundaries. These boundaries are useful when you want to give some physical meaning to your parameters, for example. However, you have to be careful with these and not provide stupid ones, *e.g.*:
...
lines(x, predict(fit_constr), col="royalblue", lwd=2)
```
...
fit3 <- nls(data=df,
            y ~ myfunc(x, y0, x0, A, B),
            start=list(y0=-10, x0=10, A=10000, B=2000)
            )
```
...
It is then easy to make a recursive fit on your data without using a `for loop`{.R}:
...
In case you want to provide fit parameters that vary depending on the group you are looking at, use the notation `.$column_name`{.R}, like is done above.
