...
You will find <a href="Data/XPdata.zip" download target="_blank">here</a> the corresponding experimental data.
...
Now let's read the data, store them in the tibble `spec` together with other informations, such as the name of the file, the fitted curves of the peaks, or the obtained pressure (the function `Pruby()`{.R} is defined in `myfunc.R`).
...
data <- tibble(name = frubis) %>%
    mutate(data = purrr::map(name,
          ...
          P = purrr::map(tidied, ~round(Pruby(.$estimate[.$term=='x1']),2))
        ) %>% 
    unnest(P)
