...
x <- c(1:12)
```
What is the value of: `length(x)`{.R}
...
a <- c(12:5)
```
What is the value of: `is.numeric(a)`{.R}
...
```
What is the value of: `which(!is.finite(x/y))`{.R}?
...
```
What is the value of: `is.character(x)`{.R}?
...
What will be the third value in the index vector operation `x[c(2, 4, 4)]`{.R}?

1. "h"
1. NA
1. "g"

What will be the fourth value in the index vector operation `x[-2]`{.R}?
...
What will be the output for the R expression `max(a[b])`{.R}?
