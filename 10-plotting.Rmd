...
- Download <a href="Data/rubis_01.txt" download target="_blank">rubis_01.txt</a>, <a href="Data/rubis_02.txt" download target="_blank">rubis_02.txt</a>, <a href="Data/rubis_03.txt" download target="_blank">rubis_03.txt</a> and <a href="Data/rubis_04.txt" download target="_blank">rubis_04.txt</a> and load them into a tidy `tibble`.
...
- Looking at data for increasing pressures
    ...
    - Plot the data using a 3D color map. Since the data are not on a regular grid, you will need to interpolate the data on a regular grid with the `akima` package and its `interp()` function
...
alldata %>% 
    ...
    ggplot(aes(x=w, 
        ...
        annotate("text", size=5, x=1780, y=seq_along(allfiles)-1, 
                label=paste(unique(alldata$P),"GPa"))+#show the pressure values
