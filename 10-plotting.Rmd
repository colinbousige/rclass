...
A plot can be exported if surrounded by `XXX` and `dev.off()`{.R}, with `XXX` that can be `pdf("xxx.pdf",height=6, width=8)`{.R}, `png("xxx.png",height=600, width=800)`{.R}... Example:
```r
...
Actually, `ggplot2` is attached to `tidyverse` so a simple `library(tidyverse)`{.R} is enough.
