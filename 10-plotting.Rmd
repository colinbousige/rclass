...
A plot can be exported if surrounded by `XXX` and `dev.off()`{.R}, with `XXX` that can be `pdf("xxx.pdf",height=6, width=8)`{.R}, `png("xxx.png",height=6, width=8)`{.R}... Example:
```r
...
You can also export the graph as a `.tex` file using `tikz()`{.R}, which allows you to use $\LaTeX$ mathematical expressions (don't forget to escape the `\` character):
```r
library(tikzDevice)
...
Further reading [here](https://ggplot2-book.org/introduction.html) and on the [cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/data-visualization-2.1.pdf) for example.
...
A quick plot can be made using the function `qplot` (for "quickplot"), very similar to the base graphics `plot` function. It's great for allowing you to produce plots quickly, but it is highly recommended learning `ggplot()`{.R} as it makes it easier to create complex graphics. 
...
It is often easier to grasp a multi-variable problem by plotting all our data in a facet plot using `facet_wrap()`{.R}:
```{r, warning = FALSE, message=FALSE}
...
If you want to make an inset plot, first make your plots, then add a plot within the other using `annotation_custom()`{.R}, specifying the x and y positions of the 4 corners of the inset plot:
