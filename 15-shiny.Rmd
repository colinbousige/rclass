...
- `sliderInput("name_of_slider", "text to display", min=min_value, max=max_value, value=current_value, step=step_value)`{.R} creates a slider to input a value. This value will be retrieved by `input$name_of_slider`{.R} in the `server()`{.R} function.
...
- write the corresponding pressure on the page using the `Pruby()`{.R} function defined in `myfunc.R` found in <a href="https://github.com/colinbousige/Rclass/raw/master/Data/XPdata.zip" download target="_blank">XPdata.zip</a>.
